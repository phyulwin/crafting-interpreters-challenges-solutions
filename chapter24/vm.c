 ...
 * ----------------------------
 * This file contains modifications for academic purposes based on
 * Chapter 24 (Calls and Functions) of "Crafting Interpreters".
 *
 * The changes include:
 * - Instruction pointer (ip) optimization
 * - Native function arity validation
 * - Native error signaling support
 * - Additional native functions
 *
 * These modifications are intended for demonstration and evaluation only.
 * This file is not guaranteed to be independently executable or production-ready.
 */
...
static bool lengthNative(int argCount, Value* args) {
  (void)argCount;
  if (!IS_STRING(args[0])) {
    return nativeError(args, "Argument must be a string.");
  }
...
static bool sqrtNative(int argCount, Value* args) {
  (void)argCount;
  if (!IS_NUMBER(args[0])) {
    return nativeError(args, "Argument must be a number.");
  }

  double value = AS_NUMBER(args[0]);
  if (value < 0) {
    return nativeError(args, "Argument must be non-negative.");
  }
...
static bool stringifyFunction(Value* args, ObjFunction* function) {
  if (function->name == NULL) {
    args[-1] = OBJ_VAL(copyString("<script>", 8));
    return true;
  }

  int length = snprintf(NULL, 0, "<fn %s>", function->name->chars);
  if (length < 0) {
    return nativeError(args, "String conversion failed.");
  }
  char* chars = ALLOCATE(char, length + 1);
  snprintf(chars, length + 1, "<fn %s>", function->name->chars);
  args[-1] = OBJ_VAL(takeString(chars, length));
  return true;
}

static bool toStringNative(int argCount, Value* args) {
  (void)argCount;
  Value value = args[0];
  if (IS_STRING(value)) {
    args[-1] = value;
  ...
  if (IS_BOOL(value)) {
    if (AS_BOOL(value)) {
      args[-1] = OBJ_VAL(copyString("true", 4));
    } else {
      args[-1] = OBJ_VAL(copyString("false", 5));
    }
    return true;
  }
  ...
  if (IS_NUMBER(value)) {
    int length = snprintf(NULL, 0, "%.15g", AS_NUMBER(value));
    if (length < 0) {
      return nativeError(args, "String conversion failed.");
    }
    char* chars = ALLOCATE(char, length + 1);
    snprintf(chars, length + 1, "%.15g", AS_NUMBER(value));
    args[-1] = OBJ_VAL(takeString(chars, length));
    return true;
  }
  ...
  switch (OBJ_TYPE(value)) {
    case OBJ_BOUND_METHOD:
      return stringifyFunction(args,
                               AS_BOUND_METHOD(value)->method->function);
    case OBJ_CLASS:
      args[-1] = OBJ_VAL(AS_CLASS(value)->name);
      return true;
    case OBJ_CLOSURE:
      return stringifyFunction(args, AS_CLOSURE(value)->function);
    case OBJ_FUNCTION:
      return stringifyFunction(args, AS_FUNCTION(value));
    case OBJ_INSTANCE: {
      ObjInstance* instance = AS_INSTANCE(value);
      int length = snprintf(NULL, 0, "%s instance",
                            instance->klass->name->chars);
      if (length < 0) {
        return nativeError(args, "String conversion failed.");
      }
      char* chars = ALLOCATE(char, length + 1);
      snprintf(chars, length + 1, "%s instance",
               instance->klass->name->chars);
      args[-1] = OBJ_VAL(takeString(chars, length));
      return true;
    }
    case OBJ_NATIVE:
      args[-1] = OBJ_VAL(copyString("<native fn>", 11));
      return true;
    case OBJ_STRING:
      args[-1] = value;
      return true;
    case OBJ_UPVALUE:
      args[-1] = OBJ_VAL(copyString("upvalue", 7));
      return true;
  }

  return nativeError(args, "Unsupported value.");
}
//< Calls and Functions clock-native
//> reset-stack
...
//> call-native
      case OBJ_NATIVE: {
        ...
        if (!native->function(argCount, args)) {
          Value error = args[-1];
          if (IS_STRING(error)) {
