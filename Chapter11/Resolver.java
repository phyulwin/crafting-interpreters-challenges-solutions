...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  // Each scope maps variable name -> [defined (Boolean), used (Boolean), Token]
  // We use a wrapper class to track both status and the token for error reporting.
  private static class VarInfo {
    final Token token;
    boolean defined;
    boolean used;

    VarInfo(Token token, boolean defined) {
      this.token   = token;
  ...
  private void endScope() {
    ...
    // Report any local variable that was never used.
  ...
  private void resolveLocal(Expr expr, Token name) {
    for (int i = scopes.size() - 1; i >= 0; i--) {
      if (scopes.get(i).containsKey(name.lexeme)) {
        scopes.get(i).get(name.lexeme).used = true;   // mark as used
