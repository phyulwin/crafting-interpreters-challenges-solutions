/*
 * Assignment Note:
 * Fixes loop variable capture so each iteration has its own scope.
 * Ensures closures capture separate values instead of sharing one.
...
// UPDATED FOR CHALLENGE 2
  ...
  // emitLoop(loopStart);
    ...
    emitLoop(loopStart);
// Changes made - Added beginScope() and endScope() function calls to give each iteration a new variable scope.
// This lets each closure capture its own separate value.
