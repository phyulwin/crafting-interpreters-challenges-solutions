 ...
 * ----------------------------
 * Implements optimization to avoid unnecessary closure creation.
 * Functions without upvalues remain ObjFunction instead of ObjClosure.
...
//< Closures call-value-closure
      case OBJ_FUNCTION: {
        // Support calling plain functions without upvalues
        ObjFunction* function = AS_FUNCTION(callee);
        return call(newClosure(function), argCount);
      }
...
//> Closures interpret-closure
      case OP_CLOSURE: {
        ...
        // Optimization: avoid creating closures when no upvalues exist
...
*/
//> Closures interpret
  // Only create closure if function actually captures variables
  if (function->upvalueCount > 0) {
